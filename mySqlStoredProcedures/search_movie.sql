DELIMITER //

CREATE DEFINER=`root`@`localhost` PROCEDURE `search_movie`(
movieStatus INT,
searchField TEXT,
searchQuery TEXT
)
BEGIN
	IF searchField = 'Title' THEN
		SELECT all_movies.title, all_movies.director, all_movies.genres
        FROM all_movies
        WHERE all_movies.title LIKE CONCAT('%', searchQuery,'%') AND all_movies.status = movieStatus AND searchQuery != '';
		
	ELSEIF searchField = 'Director' THEN
		SELECT all_movies.title, all_movies.director, all_movies.genres
        FROM all_movies
        WHERE all_movies.director LIKE CONCAT('%', searchQuery,'%') AND all_movies.status = movieStatus AND searchQuery != '';
		
	ELSEIF searchField = 'Genre' THEN
		SET @genreQueryLength = LENGTH(searchQuery);
		CREATE TABLE tempMatchedGenre(title TEXT, director TEXT, genres TEXT);
			parseLoop: LOOP
				#strip leading and trailing white spaces
				SET @genreName = LOWER(TRIM(SUBSTRING_INDEX(searchQuery, ',', 1)));
				SET @genreNameLength = LENGTH(@genreName);
				
				INSERT INTO tempMatchedGenre 
				SELECT all_movies.title, all_movies.director, all_movies.genres
				FROM all_movies
				WHERE all_movies.genres LIKE CONCAT('%', @genreName, '%') AND all_movies.status = movieStatus AND @genreName != '';
				
				SET searchQuery = MID(searchQuery, @genreNameLength + 2, @genreQueryLength);
				IF searchQuery IS NULL OR searchQuery = '' THEN
					LEAVE parseLoop;
				END IF;
			END LOOP;
			SELECT DISTINCT * FROM tempMatchedGenre;
			DROP TABLE tempMatchedGenre;
	END IF;
END//

DELIMITER ;